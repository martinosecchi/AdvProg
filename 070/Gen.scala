...
object Gen {
  ...
  def weighted[A](g1: (Gen[A],Double), g2: (Gen[A],Double)): Gen[A] = {
    ...
    double.flatMap( d => if(d < g1Prob) g1._1 else g2._1 )
  }
...
case class Prop (run :(TestCases,RNG) => Result) {
  ...
  def && (that :Prop) :Prop = Prop {
    (n, rng) => run(n, rng) match {
      case Passed | Proved => that.run(n,rng) //in case passes => whatever does the other
      case x => x //otherwise if it fails => it fails
    }
  }

  def || (that :Prop) :Prop = Prop {
    (n, rng) => run(n,rng) match {
      case Falsified(_,_) => that.run(n,rng) //in case it fails => whatever does the other
      case x => x //otherwise passes => passes
    }
  }
