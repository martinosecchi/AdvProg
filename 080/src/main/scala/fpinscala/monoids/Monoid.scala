...
object Monoid {
  ...
  def foldMapV[A,B] (v: IndexedSeq[A], m: Monoid[B]) (f: A => B) :B = v.length match {
	  case 0 => m.zero
	  case 1 => f(v.head)
	  case _ => m.op(foldMapV(v.splitAt(v.length/2)._1, m)(f), foldMapV(v.splitAt(v.length/2)._2, m)(f))
  }
...
trait Foldable[F[_]] {
  ...
  def toList[A] (fa: F[A]) :List[A] = foldRight[A, List[A]](fa)(Nil)((a: A, b: List[A]) => a :: b)
}
