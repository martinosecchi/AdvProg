...
object data {
  ...
  trait Reduce[F[_]] {
    ...
    def toTree[A] (fa :F[A]) :FingerTree[A] = {
      reduceR[A, FingerTree[A]]( (a,b) => FingerTree.addL(a,b) )(fa, Empty()) // concatenate fingertrees?
    }
  }
  ...
  sealed trait FingerTree[+A] {
    ...
    // Uncomment them once you have implemented the corresponding functions.
     ...
     def headL :A = FingerTree.headL[A] (this)
...
//     def tailR :FingerTree[A] = FingerTree.tailR (this)
    ...
    //
    def empty :Boolean = FingerTree.isEmpty(this)
    def nonEmpty :Boolean = !FingerTree.isEmpty(this)
  }
...
//      case Deep(pr,m,sf) => Some(pr.head, t)
//    }
  }
  ...
  object FingerTree extends Reduce[FingerTree] { // uncomment once the interface is implemented
    ...
    def reduceR[A,Z] (opr: (A,Z) => Z) (t: FingerTree[A], z: Z) :Z = t match {
      ...
      case Deep(pr, m, sf) => {
        ...
        def r( dig: Digit[A], z: Z) : Z = Digit.reduceR(opr)(dig,z)
        def rr( ft: FingerTree[Node[A]], z: Z ) : Z =
          reduceR[Node[A], Z](  (a,b) => Node.reduceR(opr)(a, b)  )(ft, z)
    ...
    def reduceL[A,Z] (opl: (Z,A) => Z) (z: Z, t: FingerTree[A]) :Z = t match {
      ...
      case Deep(pr, m, sf) => {
        ...
        def r (z:Z, dig: Digit[A] ) : Z = Digit.reduceL(opl)(z,dig)
        def rr (z: Z, ft : FingerTree[Node[A]]) : Z =
          reduceL[Node[A], Z]( (b,a) => Node.reduceL(opl)(b,a) )(z,ft)
    ...
    def addL[A] (a: A, t: FingerTree[A]) : FingerTree[A] = t match {
      ...
      case Deep(pr,m,sf) => pr match {
        case Digit(b,c,d,e) => Deep(Digit(a,b), addL[Node[A]](Node3(c,d,e), m), sf)
    ...
    def addR[A] (t: FingerTree[A], a: A) : FingerTree[A] = t match {
      ...
      case Deep(pr, m, sf) => sf match {
        case Digit(e,d,c,b) => Deep(pr, addR[Node[A]](m, Node3(e,d,c)), Digit(b,a))
        case _ => Deep(pr, m, sf:+a) //:+ adds elem in tail of a list
      }
    }
    ...
    def deepL[A] (pr: Digit[A], m: FingerTree[Node[A]], sf: Digit[A]) : FingerTree[A] =
      pr match {
        case Digit() => viewL(m) match {
          case NilTree() =>  ??? //toTree(sf)
          case ConsL(aa, mm) => ??? //Deep[A]( aa , mm, sf)
        }
        case _ => Deep(pr, m, sf)
      }
    ...
    def isEmpty[A]( t: FingerTree[A]) : Boolean = viewL(t) match {
      case NilTree() => true
      case ConsL(_,_) => false
    }

     def headL[A](t: FingerTree[A]) = viewL(t) match {
       case ConsL(a, _) => a
     }
     def tailL[A](t: FingerTree[A]) = viewL(t) match {
       case ConsL(_,t) => t
     }
     def headR[A](t: FingerTree[A]) = ???
     def tailR[A](t: FingerTree[A]) = ???
  }
